...
export default function SubjectManager() {
  ...
  function handleAdd() {
    ...
    const subject = {
      name: form.name,
      masterPdf: form.masterPdf.name,
      studentPdfs: form.studentPdfs.map(f => f.name),
  ...
  function handleEdit(idx) {
    ...
    setForm({ name: s.name, masterPdf: { name: s.masterPdf }, studentPdfs: s.studentPdfs.map(n => ({ name: n })) });
    setEditIndex(idx);
  }
      ...
      <div className={styles.layout}>
              ...
              {state.subjects.map((subject, idx) => (
                <li key={idx} className={styles.subjectCard}>
                  <div className={styles.subjectCardTop}>
                    ...
                    <div className={styles.subjectInfo}>
                      ...
                      <span className={styles.subjectMeta}>
                        Master: {subject.masterPdf} · {subject.studentPdfs.length} student(s)
                      </span>
                    </div>
                  ...
                  <div className={styles.subjectTags}>
                    {subject.studentPdfs.slice(0, 3).map((pdf, i) => (
                      <span key={i} className={styles.pdfTag}>{pdf}</span>
                    ))}
                    {subject.studentPdfs.length > 3 && (
                      <span className={styles.pdfTagMore}>+{subject.studentPdfs.length - 3} more</span>
                    )}
                  </div>
                </li>
              ))}
        ...
        <div className={styles.formPanel}>
          <div className={styles.panelHeader}>
            ...
            {editIndex !== null && (
              <button className={styles.cancelBtn} onClick={() => { setEditIndex(null); setForm({ name: '', masterPdf: null, studentPdfs: [] }); }}>
                Cancel
              </button>
            )}
          </div>
          ...
          <div className={styles.formBody}>
                  ...
                  {form.studentPdfs.map((f, i) => (
                    <div key={i} className={styles.studentPdfItem}>
                      ...
                      <button
                        ...
                        onClick={() => setForm(prev => ({ ...prev, studentPdfs: prev.studentPdfs.filter((_, j) => j !== i) }))}
                      >×</button>
                    </div>
                  ))}
                </div>
            ...
            <div className={styles.formActions}>
              ...
              <button className={styles.secondaryBtn} onClick={() => setForm({ name: '', masterPdf: null, studentPdfs: [] })}>
                Reset
              </button>
            </div>
            ...
            <div className={styles.csvActions}>
              <button className={styles.csvBtn}>📥 Import from CSV</button>
              <button className={styles.csvBtn}>📤 Export to CSV</button>
            </div>
          </div>
        </div>
      </div>
