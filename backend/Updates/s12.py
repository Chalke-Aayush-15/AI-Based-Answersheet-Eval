...
class MultiSubjectFairEvaluator:
    ...
    def __init__(self, use_ocr=False, exam_mode='unit_test', roster_path=None):
        ...
        # ─── Exam mode: 'unit_test' (details inside PDF) or 'end_sem' (seat no → Excel lookup) ───
    ...
    # ================================================================
    # END-SEM SUPPORT: Roster loading + seat-number extraction/lookup
    # ================================================================
    def load_roster(self, roster_path):
        """Load the student roster Excel file used in End-Sem mode.
        Expected columns (case-insensitive, flexible): Seat no, Name, Gmail id.
        """
        try:
            df = pd.read_excel(roster_path)
            # Normalize column names → lowercase stripped keys
            ...
            self.roster_df = df
            self.roster_path = roster_path
            self.log_messages.append(f"✅ Roster loaded: {len(df)} students from {os.path.basename(roster_path)}")
        except Exception as e:
            self.roster_df = None
            self.log_messages.append(f"❌ Failed to load roster: {e}")
            raise

    def extract_seat_number(self, text):
        """Extract seat number from an End-Sem answer sheet text (OCR output)."""
        if not text:
            return ''
        patterns = [
        ...
        # Fallback: try filename-embedded seat no (handled by caller)
        return ''

    def _extract_seat_from_filename(self, pdf_path):
        base = os.path.basename(pdf_path)
        m = re.search(r'(?:seat[_\s-]*(?:no|number)?[_\s-]*)?([0-9]{4,10})', base, re.IGNORECASE)
        return m.group(1) if m else ''

    def lookup_student_by_seat(self, seat_no):
        """Look up a student record in the loaded roster by seat number.
        Returns dict {name, roll_no, email} or None if not found.
        """
        if self.roster_df is None or not seat_no:
            return None
        seat_no = str(seat_no).strip()
        df = self.roster_df
        match = df[df['seat_no'] == seat_no]
        if match.empty:
            # numeric fallback
            try:
                match = df[df['seat_no'].str.lstrip('0') == seat_no.lstrip('0')]
            except Exception:
                pass
        if match.empty:
            return None
    ...
    def evaluate_subject(self, subject_data, log_widget=None):
        ...
        for i, student_pdf in enumerate(subject_data.student_pdfs):
            ...
            # ─── END-SEM MODE: replace details via seat-no → roster lookup ───
            if self.exam_mode == 'end_sem':
                seat = self.extract_seat_number(student_text) or self._extract_seat_from_filename(student_pdf)
                if not seat:
                    self.log(f"  ⚠️ End-Sem: could not detect seat number in {filename}", log_widget)
                else:
                    self.log(f"  🎫 End-Sem seat number detected: {seat}", log_widget)
                    record = self.lookup_student_by_seat(seat)
                    if record:
                        student_data['name'] = record['name']
                        student_data['roll_no'] = record['roll_no']
                        student_data['email'] = record['email'] or student_data.get('email', '')
                        self.log(f"  ✅ Roster match: {record['name']} <{record['email']}>", log_widget)
                    else:
                        self.log(f"  ⚠️ Seat {seat} not found in roster — using seat as roll no", log_widget)
                        student_data['name'] = f'Seat_{seat}'
                        student_data['roll_no'] = seat
                        student_data['email'] = ''
...
class MultiSubjectGUI:
    ...
    def __init__(self, root):
        ...
        # ─── Exam mode selection ───
        # 'unit_test' → extract student details from PDF itself
        # 'end_sem'   → extract seat number, look up details from Excel roster
        self.exam_mode = tk.StringVar(value='unit_test')
        self.roster_path = tk.StringVar(value='')
    ...
    def create_evaluation_tab(self):
        ...
        # ─── Exam Mode selector ───
        mode_frame = ttk.LabelFrame(options_frame, text="📝 Exam Type", padding=8)
        mode_frame.pack(fill='x', pady=(0, 8))

        ttk.Radiobutton(
            mode_frame,
            text="Unit Test  (student name / roll / email inside answer sheet)",
            variable=self.exam_mode,
            value='unit_test',
            command=self._on_exam_mode_change
        ).pack(anchor='w')

        ttk.Radiobutton(
            mode_frame,
            text="End Sem   (only seat no on sheet → look up student in Excel roster)",
            variable=self.exam_mode,
            value='end_sem',
            command=self._on_exam_mode_change
        ).pack(anchor='w')

        # Roster picker (visible/enabled only in End-Sem mode)
        roster_row = ttk.Frame(mode_frame)
        roster_row.pack(fill='x', pady=(6, 0))
        ttk.Label(roster_row, text="Student Roster (Excel):", font=("Arial", 9)).pack(side='left')
        self.roster_entry = ttk.Entry(roster_row, textvariable=self.roster_path, width=32, font=("Arial", 9))
        self.roster_entry.pack(side='left', padx=6, fill='x', expand=True)
        self.roster_browse_btn = ttk.Button(roster_row, text="Browse", width=8, command=self.browse_roster_file)
        self.roster_browse_btn.pack(side='left')
        self._on_exam_mode_change()
...
"""
    ...
    def _on_exam_mode_change(self):
        """Enable/disable roster picker based on selected exam mode."""
        try:
            end_sem = (self.exam_mode.get() == 'end_sem')
            state = 'normal' if end_sem else 'disabled'
            if hasattr(self, 'roster_entry'):
                self.roster_entry.config(state=state)
            if hasattr(self, 'roster_browse_btn'):
                self.roster_browse_btn.config(state=state)
        except Exception:
            pass

    def browse_roster_file(self):
        """Pick the Excel roster file used in End-Sem mode."""
        path = filedialog.askopenfilename(
            title="Select Student Roster (Excel)",
    ...
    def start_evaluation(self):
        ...
        # ─── Exam mode / roster wiring ───
        mode = self.exam_mode.get()
        self.evaluator.exam_mode = mode
        if mode == 'end_sem':
            roster = self.roster_path.get().strip()
            if not roster or not os.path.exists(roster):
                messagebox.showerror(
                    "Roster required",
                    "End-Sem mode needs a Student Roster Excel file.\n"
                    "Please browse to a .xlsx with columns: Seat no, Name, Gmail id."
                )
                return
            try:
                self.evaluator.load_roster(roster)
            except Exception as e:
                messagebox.showerror("Roster error", f"Failed to load roster:\n{e}")
                return
            self.log_message(f"📋 End-Sem mode: roster loaded ({len(self.evaluator.roster_df)} students)", "info")
        else:
            self.evaluator.roster_df = None
            self.evaluator.roster_path = None
            self.log_message("📝 Unit-Test mode: extracting student details from answer sheets", "info")
